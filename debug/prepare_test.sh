...
# Build YCSB
cd $ycsb_git
mvn -pl site.ycsb:mdde.redis-binding -am clean package -DskipTests

# Move YCSB to the test destination folder
ycsb_bin=../ycsb
ycsb_dist="$(ls  mdde.redis/target/ycsb-mdde.redis-*.tar.gz | sort -V | tail -n1)"
if [ ! -f "$ycsb_dist" ]; then
