...
# Build (re-build) default DQN-singleagent and all supporting images
...
if [ $SKIPENVBASE -eq 0 ];then
  (cd ../../../images/environment/scripts/ && sh build_dqn-ray-singleagent.sh)
else
  (cd ../../../images/environment/scripts/ && sh build_dqn-ray-singleagent.sh -b)
fi
...
(cd .. && docker-compose -f docker-compose.yml -f docker-compose.ray-dqn-singleagent.yml build --no-cache)
