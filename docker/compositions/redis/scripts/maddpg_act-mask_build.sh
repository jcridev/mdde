...
if [ $SKIPENVBASE -eq 0 ];then
  (cd ../../../images/environment/scripts/ && sh build_maddpg-actmask-ray.sh)
else
  (cd ../../../images/environment/scripts/ && sh build_maddpg-actmask-ray -b)
fi
...
(cd .. && docker-compose -f docker-compose.yml -f docker-compose.maddpg-actmask.yml build --no-cache)
