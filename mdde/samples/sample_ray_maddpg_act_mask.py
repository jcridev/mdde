...
class MADDPGSampleActMask:
    """
    Demonstration sample code showing how RAY's MADDPG can be executed used with MDDE with Action masks.

    Note: to run this sample, you need to modify Ray's MADDPG code locally. Refer to ./maddpg_masked_act for more
    details.
    """
    ...
    def run_maddpg(self, config):
        ...
        MddeMultiAgentEnvActionMask.configure_ray(ray)

        maddpg_agent = MADDPGTrainer.with_updates(
            mixins=[MADDPGSampleActMask.CustomStdOut]
        )
        ...
        env_instance = MddeMultiAgentEnvActionMask(env=make_env(host=self.mdde_registry_host,
        ...
        def env_creator(kvargs):
            env = make_env(**kvargs)
            return MddeMultiAgentEnvActionMask(env=env,
                                               observation_shaper=sample_selected_shaper)
...
if __name__ == '__main__':
    ...
    MADDPGSampleActMask.run_result_dir = config.result_dir
    MADDPGSampleActMask.ray_temp_dir = config.temp_dir

    MADDPGSampleActMask.mdde_registry_host = config.reg_host
    MADDPGSampleActMask.mdde_registry_port = config.reg_port
    MADDPGSampleActMask.mdde_registry_config = config.config

    MADDPGSampleActMask.env_temp_dir = config.env_temp_dir

    runner = MADDPGSampleActMask()
    runner.setUp()
