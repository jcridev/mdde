...
def before_learn_on_batch(multi_agent_batch, policies, train_batch_size):
    ...
    for pid, p in policies.items():
        i = p.config["agent_id"]
        d_keys = multi_agent_batch.policy_batches[pid].data.keys()
        keys = ["_".join([k, str(i)]) for k in d_keys]
        policy_samples = {}
        for k, v in multi_agent_batch.policy_batches[pid].data.items():
            if 'obs' in k:
                samples["_".join([k, str(i)])] = v[:, : p._act_mask_len]
                samples["_".join(['action_mask', str(i)])] = v[:, p._act_mask_len: ]
            else:
                samples["_".join([k, str(i)])] = v
#        samples.update(
#            dict(
#                zip(keys,
