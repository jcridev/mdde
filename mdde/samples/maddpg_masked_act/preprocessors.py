...
class DictFlatteningPreprocessorActionMask(DictFlatteningPreprocessor):  # MDDE_MARK
    """Preprocesses each dict value, then flattens it all into a vector.
    ActionMask will be put into the end of the space.

    RLlib models will unpack the flattened output before _build_layers_v2().
    """

    @override(DictFlatteningPreprocessor)  # MDDE_MARK
    def write(self, observation, array, offset):  # MDDE_MARK
        if not isinstance(observation, OrderedDict):
            observation = OrderedDict(sorted(observation.items()))
        assert len(observation) == len(self.preprocessors), \
            (len(observation), len(self.preprocessors))
        action_mask_val = None
        action_mask_p = None
        for i, p in zip(observation.items(), self.preprocessors):
            if i[0] == 'action_mask':
                action_mask_val = i[1]
                action_mask_p = p
                continue
            p.write(i[1], array, offset)
            offset += p.size
        if action_mask_val is not None:
            action_mask_p.write(action_mask_val, array, offset)
            offset += action_mask_p.size
...
def get_preprocessor(space):
    ...
    elif isinstance(space, gym.spaces.Dict) and 'action_mask' in space.spaces.keys():  # MDDE_MARK
        preprocessor = DictFlatteningPreprocessorActionMask  # MDDE_MARK
